...
-- VOTRE RÉPONSE 
/*Go
CREATE VIEW Joueurs.vw_StatistiquesJoueur AS
Select M.Nom, M.Prenom, J.Rang, S.NbPartiesJouees, S.NbPartiesGagnees, S.NbTournoisJoues, s.NbTournoisGagnes, J.JoueurID, M.MembreID, S.StatistiquesAnneeJoueurID
...
-- VOTRE RÉPONSE 
/*Select Sum(S.NbTournoisGagnes) as [Nb tournois gagnés rang 6DAN]
from Joueurs.vw_StatistiquesJoueur S
where Rang = '6DAN'*/
...
/*
Select S.Nom, s.Prenom ,Sum(ISNULL( S.NbTournoisGagnes,0)) as [Nb Tournois Gagnés ]
from Joueurs.vw_StatistiquesJoueur S
group by S.Prenom , S.Nom
HAVING Sum(ISNULL( S.NbTournoisGagnes,0)) > 2
order by  [Nb Tournois Gagnés ] desc;
*/
